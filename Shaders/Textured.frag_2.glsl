...
void main()
{
    const float ZOOM = 2.6;
     float FORMUPARAM = abs(fract(time*0.003)*1.0 - 1.0);
    const int ITERATIONS = 45;
    ...
    for(
        ...
        {
            float s = float(i) * .035;
            vec3 p = s * vec3(uv, 1.0 + sin(0.*time * .015));
            p.xy *= ma;
    ...
    vec3 col = vec3(re - 0.2, gr, bl) + smoothstep(0.15, .0, len) * 0.9;
